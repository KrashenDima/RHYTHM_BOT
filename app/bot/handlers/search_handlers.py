...
async def get_random_profile(profile_record: ProfilesModel,
    ...
    if not appropriate_profile:
        ...
        appropriate_profile_id = await db.users.get_telegram_id_from_profile(
            user_id=appropriate_profile[0])
...
async def process_like(message: Message, 
    ...
    try:
        ...
        if is_match:
            await message.answer(text=LEXICON_RU['match'])
            await message.bot.send_message(chat_id=to_user, text=LEXICON_RU['match'])
